...
module.exports = (pool) => {
    ...
    router.get('/create', async (req,res) => {
        try {
            await pool.query(`
                CREATE TABLE ThuongHieu(ID_ThuongHieu VARCHAR(10) PRIMARY KEY, Ten VARCHAR(10), SMS VARCHAR(11), MatKhau VARCHAR(20) NOT NULL, HinhAnh VARCHAR(300), LinhVuc VARCHAR(30), DiaChi VARCHAR(100), KinhDo VARCHAR(30), ViDo VARCHAR(30), TrangThai BOOLEAN);
                CREATE TABLE Game(ID_Game VARCHAR(10) PRIMARY KEY, Ten VARCHAR(10));
                CREATE TABLE SuKien(ID_SuKien VARCHAR(10) PRIMARY KEY, ID_ThuongHieu VARCHAR(10), ID_Game VARCHAR(10), TenSuKien VARCHAR(100), HinhAnh VARCHAR(300), SoLuongVoucher INT, MoTa TEXT, ThoiGianBatDau TIMESTAMP WITH TIME ZONE, ThoiGianKetThuc TIMESTAMP WITH TIME ZONE, CONSTRAINT FK_SuKien_ThuongHieu FOREIGN KEY (ID_ThuongHieu) REFERENCES ThuongHieu (ID_ThuongHieu), CONSTRAINT FK_SuKien_Game FOREIGN KEY (ID_Game) REFERENCES Game (ID_Game));
                CREATE TABLE Voucher(ID_Voucher VARCHAR(10) PRIMARY KEY, Ten VARCHAR(100), QRCode VARCHAR(300), HinhAnh VARCHAR(300), TriGia INT, MoTa TEXT, NgayHetHan TIMESTAMP WITH TIME ZONE, TrangThai BOOL, ID_ThuongHieu VARCHAR(10), CONSTRAINT FK_Voucher_ThuongHieu FOREIGN KEY (ID_ThuongHieu) REFERENCES ThuongHieu(ID_ThuongHieu)); 
    ...
    router.get('/insert', async (req,res) => {
        try {
            await pool.query(`
                INSERT INTO ThuongHieu VALUES ('ID00000001', N'Aquafina', '0123456789', '123', 'https://rubicmarketing.com/wp-content/uploads/2022/12/logo-aquafina.jpg', N'Nước giải khát', N'123 Nguyễn Văn Cừ, Quận 5, TP.HCM', '106 0 22’ – 106 054 ’', '10 0 10’ – 10 0 38', true);
